...
class ActionCriticModel(nn.Module):
    ...
    def forward(self, state_vect, act):
        b = state_vect.shape[:1].numel()
        state_vect = state_vect.reshape(b, self.agent_num, 4, -1)
        state_pos = state_vect[:, :, :, :5].reshape(b, self.agent_num, -1)
        state_ray = state_vect[:, :, :, 5:].reshape(b, self.agent_num, -1)
        act = act.reshape(b, self.agent_num, -1)

        ray_feature = self.fc_head_ray(state_ray)
        pos_feature = self.fc_head_pos(state_pos)
        act_feature = self.fc_head_act(act)
        common_vect = torch.cat([pos_feature, act_feature, ray_feature], dim=-1)
        assert common_vect.shape[:2] == torch.Size([b, self.agent_num])

        out = self.trans_block1(nn.functional.relu(common_vect))
