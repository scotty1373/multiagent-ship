...
class MATD3:
    ...
    def get_action(self, vect_state):
        ...
        for idx in range(self.agent_num):
            actions.append(self.agent_list[idx].get_action(vect_state[idx]))
        return actions
    ...
    def update(self, replay_buffer):
        ...
        for idx in range(self.agent_num):
            ...
            agent_actor_action.append(self.agent_list[idx].get_action(actor_vect[idx]))
        ...
        # critic更新
        for idx in range(self.agent_num):
            critic_loss = self.critic_update(idx, vect_state, next_vect_state, agent_target_action, agent_online_action, reward, done)

            # actor延迟更新
            if self.t % self.delay_update == 0:
                self.action_update(vect_state=vect_state, actor_action=agent_actor_action, agent_idx=idx)
                with torch.no_grad():
                    self.agent_list[idx].model_soft_update(self.agent_list[idx].actor_model, self.agent_list[idx].actor_target)
                    self.agent_list[idx].model_soft_update(self.agent_list[idx].critic_model, self.agent_list[idx].critic_target)
    ...
    def action_update(self, vect_state, actor_action, agent_idx):
        critic_q1 = self.agent_list[agent_idx].critic_model.q_theta1(vect_state, actor_action)
